...
func PromptLibrary(r *ghttp.Request) {
	...
	headerMap["Authorization"] = r.GetHeader("Authorization")
	ProxyClient := gclient.New().SetHeaderMap(headerMap)
	res, err := ProxyClient.Get(ctx, config.GatewayUrl+"/backend-api/prompt_library/", g.Map{"limit": limit, "offset": offset})
	if err != nil {
		g.Log().Error(ctx, err)
